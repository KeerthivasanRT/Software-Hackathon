    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1 font-medium">Submit and track your transport feedback.</p>
        </div>
        
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button className="bg-sky-600 hover:bg-sky-700 text-white rounded-lg shadow-sm hover:shadow-md transition-all h-10 px-4">
          ...
          <DialogContent className="sm:max-w-[500px] bg-white rounded-2xl p-0 overflow-hidden border-0 shadow-2xl">
            ...
            <div className="p-6 space-y-5">
              <div className="space-y-2">
                ...
                <Input className="h-11 rounded-lg border-[#D6ECFA] focus-visible:ring-sky-500/20" value={subject} onChange={e => setSubject(e.target.value)} placeholder="e.g. Bus delayed, AC issue" />
              </div>
              <div className="space-y-2">
                <Label className="text-xs font-semibold text-slate-600 uppercase tracking-wider">Description</Label>
                <textarea 
                  className="flex min-h-[120px] w-full rounded-lg border border-[#D6ECFA] bg-white px-3 py-3 text-sm text-slate-900 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky-500/20 focus-visible:border-sky-500 transition-all resize-none"
            ...
            <div className="px-6 py-4 border-t border-[#D6ECFA] bg-sky-50 flex justify-end space-x-3">
              ...
              <Button className="bg-sky-600 hover:bg-sky-700 text-white rounded-lg h-10 px-5 shadow-sm" onClick={handleSubmit}>Submit</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
              ...
              {userComplaints.map((complaint) => (
                <TableRow key={complaint.id} className="hover:bg-sky-50/80 transition-colors border-b border-[#D6ECFA]">
                  ...
                  <TableCell>
                    ...
                    <div className="text-sm text-slate-600 mt-1 max-w-lg">{complaint.description}</div>
                  </TableCell>
