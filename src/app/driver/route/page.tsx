  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-slate-600 mt-1 font-medium">View your assigned daily transport route.</p>
      </div>
              ...
              {route ? (
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-slate-600 text-sm mt-1">Bus: {assignedBus?.busNumber}</p>
                  </div>
                    ...
                    <div className="space-y-0 relative before:absolute before:inset-0 before:ml-[11px] before:-translate-x-px before:h-full before:w-0.5 before:bg-sky-100">
                      {route.stops.map((stop, i) => (
                        <div key={stop.id} className="relative flex items-start group pb-5 last:pb-0">
                          <div className={`flex items-center justify-center w-6 h-6 rounded-full border-2 border-white shadow-sm shrink-0 z-10 ${i === 0 ? 'bg-emerald-500 text-white' : i === route.stops.length - 1 ? 'bg-red-500 text-white' : 'bg-blue-100 text-sky-600'}`}>
